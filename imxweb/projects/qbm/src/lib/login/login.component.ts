...
export class LoginComponent implements OnInit, OnDestroy {
    ...
    this.subscriptions.push(this.authentication.onSessionResponse.subscribe((sessionState: ISessionState) => {
      ...
      if (this.sessionState.IsLoggedIn) {
        ...
        if (this.sessionState.configurationProviders && this.sessionState.configurationProviders.length > 0) {
          ...
          if (this.sessionState.configurationProviders[2] != null && this.sessionState.configurationProviders[2].name == "RoleBasedPerson")
          this.selectedConfigProvider = this.sessionState.configurationProviders[2]
        else 
        this.selectedConfigProvider = this.sessionState.configurationProviders.find(authProvider =>
  ...
  public onSelectAuthConfig(): void {
    ...
    //sthis.loginData = { Module: "RoleBasedPerson"}; // updated instead of  this.selectedConfigProvider.name
  ...
  private buildConfigurationProviders(): void {
    ...
    // let CCC_sortedProvider=[] ;
    // for (let element of provider){
    //   if (element.name === "RoleBasedPerson") {
    //     CCC_sortedProvider.unshift(element);
    //   } else {
    //     CCC_sortedProvider.push(element);
    //   }
    // }
